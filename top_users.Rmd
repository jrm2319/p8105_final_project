...
top_ratings_genres <- left_join(top_ratings, books_with_tags, by = c("book_id")) %>%
  ...
  filter(!tag %in% c("owned", "to-read", "favorites", "owned", "books-i-own", "currently-reading", "library", "owned-books", "to-buy", "kindle", "default", "ebook", "audiobook", "ebooks", "wish-list", "my-library", "audiobooks", "i-own", "book-club", "discworld")) %>%
    group_by(user_id) %>%
    drop_na(tag) %>%
