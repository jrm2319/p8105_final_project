...
head(duplicate_series)

sequel_clean <- sequel_data |>
  mutate(series = str_extract(title, "\\(.*\\)"), 
         series_name = str_sub(str_extract(series, '\\(.*,'), 2, -2)) |>
  group_by(series_name, series_number) |>
  summarise(
    average_rating = mean(average_rating, na.rm = TRUE), 
    ratings_count = sum(ratings_count, na.rm = TRUE),
    .groups = "drop"
  )

duplicate <- sequel_clean |>
  group_by(series_name) |>
  summarise(entries = n()) |>
  filter(entries > 1)
...
duplicate_check <- sequel_clean |>
  group_by(series_name, series_number) |>
  summarise(entries = n()) |>
  filter(entries > 1)

# View details of these duplicates
problem_series <- sequel_clean |>
  filter(series_name %in% duplicate_check$series_name)

print(problem_series)

sequel_clean <- sequel_clean |>
  group_by(series_name, series_number) |>
  slice_max(order_by = ratings_count, n = 1) |>
  ungroup()
```
...
```{r}
sequel_performance <- sequel_clean |>
  group_by(series_name) |>
